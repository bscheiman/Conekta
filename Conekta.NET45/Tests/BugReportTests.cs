...
namespace Conekta.Tests {
    ...
    public class BugReportTests {
        ...
        public void TestBugReport1() {
            ...
            Assert.AreEqual(obj.PaymentMethod.FraudIndicators[0].Description,
                "En las últimas 6 horas la persona ha intentado realizar compras con más de una tarjeta distinta; por seguridad se ha bloqueado la compra.");
            Assert.AreEqual(obj.PaymentMethod.FraudIndicators[0].Weight, 29);
            Assert.AreEqual(obj.PaymentMethod.FraudIndicators[1].Description,
                "El dispositivo con el que se hizo la compra se encuentra fuera de México y la persona tiene asociada a ella distintas tarjetas, correos, direcciones IP, etc. Este patrón es muy común en ataques; por seguridad se ha bloqueado la compra.");
            Assert.AreEqual(obj.PaymentMethod.FraudIndicators[1].Weight, null);
            Assert.AreEqual(obj.PaymentMethod.FraudIndicators[2].Description,
                "La transacción se ha aprobado a través de un pago vía la modalidad Paga Móvil de Banorte.");
